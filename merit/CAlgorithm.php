    ...
    class CAlgorithm extends CMerit {
        ...
        public function accept(&$data, &$user) {
            ...
            if (bccomp($user['param']['enableBalance'], 0) == 0 || bccomp($data['param']['cash'], 0) == 0) {
                ...
                if (bccomp($data['param']['cash'], $data['option']['minimum'] * 2) < 0 || $data['param']['people'] + 1 >= $data['option']['people']) {
                    /**
                    ...
                    if (bccomp($user['param']['enableBalance'], $data['param']['cash']) == 0) { //用户金额与标金额相同
                        ...
                        if (bccomp($user['param']['enableBalance'], $data['param']['cash']) == -1) {   //用户金额<标金额
        ...
        private function __getMerit($data, &$get) {
            ...
            if ($data['data']['let'] == true || bccomp($get['data']['cash'], $data['merit']['principal']) > -1) {
                 ...
                 *      转让金额=标的余下本金
                 */
                $get['data']['cash'] -= $currentCash = $data['merit']['principal'] >= 2 * $minimum ? ($data['data']['people'] < $data['option']['people'] ? ($get['data']['cash'] <= $minimum ? $minimum : ($get['data']['cash'] <= $data['merit']['principal'] - $minimum ? $get['data']['cash'] : $data['merit']['principal'])) : $data['merit']['principal']) : $data['merit']['principal'];
