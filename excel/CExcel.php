<?php
    ...
    class CExcel {
        ...
        private function __createSharedStrings() {
            ...
            fwrite($this->__file_data, '<?xml version="1.0" encoding="utf-8"?>' . PHP_EOL . '<sst xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">' . PHP_EOL);
            //头数据
            foreach ($this->__settings['column'] as $val) {
                fwrite($this->__file_data, '	<si>' . PHP_EOL . '		<t>' . $val . '</t>' . PHP_EOL . '	</si>' . PHP_EOL);
            }
            //列数据
        ...
        private function __createSheetData() {
            ...
            fwrite($this->__file_data, '<?xml version="1.0" encoding="utf-8"?>' . PHP_EOL . '<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships" xmlns:xdr="http://schemas.openxmlformats.org/drawingml/2006/spreadsheetDrawing" xmlns:x14="http://schemas.microsoft.com/office/spreadsheetml/2009/9/main" xmlns:mc="http://schemas.openxmlformats.org/markup-compatibility/2006">' . PHP_EOL . '	<sheetPr/>' . PHP_EOL . '	<dimension ref="A1:' . $this->__getColumnName(64 + $this->__settings['columnTotal']) . ($this->__settings['dataRow'] + $this->__settings['rowTotal']) . '" />' . PHP_EOL . '	<sheetFormatPr defaultColWidth="' . $this->__settings['columnWidth'] . '" defaultRowHeight="' . $this->__settings['rowHeight'] . '" />' . PHP_EOL);
            //列名
            ...
            for ($row = $this->__settings['dataRow'], $index = 0; $row < $this->__settings['rowTotal'] + $this->__settings['dataRow'] + 1; $row++) {
                ...
                foreach ($this->__settings['column'] as $key => $val) {
                    ...
                    fwrite($this->__file_data, '			<c r="' . $columnName . '" s="2" t="s">' . PHP_EOL . '				<v>' . $index++ . '</v>' . PHP_EOL . '			</c>' . PHP_EOL);
                }
