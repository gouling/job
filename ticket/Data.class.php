    ...
    class Data {
        ...
        public function readData($binary) {
            if($data=$this->__getArrayData($binary)) {
                if(($refer=call_user_func_array(array(D('Api/HelpTicket'), 'getRequestData'), array(
        ...
        private function __getBinaryData($payload_id, $data) {
            ...
            $refer=array(
                'sync_tag'=>$this->__encrypt16UInt(C('TICKET_INTERFACE.SYNC'), $packet_length),   //同步标记
                'version'=>$this->__encrypt8UInt(C('TICKET_INTERFACE.VER'), $packet_length),   //版本
                'packet_length'=>$this->__encrypt16UInt(0, $packet_length),   //报文总长度仅占位
                'payload_id'=>$this->__encrypt8UInt($payload_id, $packet_length), //协议体标识
                'payload_data'=>$this->__encryptData($payload_id, $data, $packet_length),   //协议体数据
                'packet_length'=>$this->__encrypt16UInt($packet_length+2, $packet_length),  //重写报文总长度(+2 循环冗余校验码长度)
            );
        ...
        private function __encryptData($payload_id, $data, &$packet_length) {
            foreach($data as $key=>$val) {
                ...
                //值是数组，查找对应的规则
                if(is_array($val)) {
                    if(isset($rule[$key]['length'])) unset($rule[$key]['length']);
        ...
        public function __destruct() {
            call_user_func_array(array(D('Api/HelpTicket'), 'logout'), array(
                'ip'=>$this->_ip
            ));
        }
    }
